...
function std_get($params = NULL) {
    ...
    if ($params != NULL) {
        ...
        if (isset($params["where_raw"])) {
            foreach ($params["where_raw"] as $row) {
                $query->whereRaw("BINARY ".$row["field_name"]." = ?",[$row["value"]]);
            }
        }
...
function std_update_where_in($params = NULL)
{
    return DB::table($params["table_name"])->whereIn($params["where_in"]["column"],$params["where_in"]["value"])->update($params["data"]);
}
